...
morpheus::chandlersession_base::chandlersession_base( morpheus * parent ):m_parent(parent),m_completed(false),m_lifetime_timer_opaque(0) {}
...
bool morpheus::chandlersession_base::handle_error (rofl::cofdpt *src, rofl::cofmsg_error *msg) { std::cout << "** Received error message from dpt " << src->c_str() << " with xid ( " << msg->get_xid() << " ): no handler implemented. Dropping it." << std::endl; return m_completed; }
...
void morpheus::chandlersession_base::setLifetimeTimerOpaque( int timer_opaque ) { m_lifetime_timer_opaque = timer_opaque; }
